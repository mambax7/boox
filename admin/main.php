...
switch ($op) {
    ...
    case 'save':        // Save a file after it was edited (or created)
        ...
        $fp = fopen($folder_storage . $filename, 'w') || die(_AM_BOOX_ERROR1);
        fwrite($fp, $content);
        fclose($fp);
    ...
    case 'edit':
        ...
        $fp       = fopen($folder_storage . $filename, 'r') || die(sprintf(_AM_BOOX_ERROR3, $filename));
    ...
    default:        // Default action, list existing files
        ...
        xoops_cp_header();
        $adminObject->displayNavigation(basename(__FILE__));
        $adminObject->addItemButton(_AM_BOOX_ADD, 'main.php?op=addfile', 'add', '');
